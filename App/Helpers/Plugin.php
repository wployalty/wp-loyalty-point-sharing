...
class Plugin {
	...
	public static function checkDependencies( bool $allow_exit = false ): bool {
		if ( ! self::isPHPCompatible() ) {
			...
			$message = sprintf( esc_html__( '%1$s requires minimum PHP version %2$s', 'wp-loyalty-point-sharing' ), WLPS_PLUGIN_NAME, WLPS_MINIMUM_PHP_VERSION );
		...
		if ( ! self::isWordPressCompatible() ) {
			...
			$message = sprintf( esc_html__( '%1$s requires minimum WordPress version %2$s', 'wp-loyalty-point-sharing' ), WLPS_PLUGIN_NAME, WLPS_MINIMUM_WP_VERSION );
		...
		if ( ! self::isActive( 'woocommerce/woocommerce.php' ) ) {
			...
			$message = sprintf( esc_html__( '%1$s requires WooCommerce to be installed and activated in order to be used.', 'wp-loyalty-point-sharing' ), WLPS_PLUGIN_NAME );
			$allow_exit ? exit( esc_html( $message ) ) : self::adminNotice( esc_html( $message ), 'error' );
		...
		if ( ! self::isWooCompatible() ) {
			...
			$message = sprintf( esc_html__( '%1$s requires minimum Woocommerce version %2$s', 'wp-loyalty-point-sharing' ), WLPS_PLUGIN_NAME, WLPS_MINIMUM_WC_VERSION );
			$allow_exit ? exit( esc_html( $message ) ) : self::adminNotice( esc_html( $message ), 'error' );
		...
		if ( ( ! self::isActive( 'wployalty/wp-loyalty-rules-lite.php' ) ) && ( ! self::isActive( 'wp-loyalty-rules/wp-loyalty-rules.php' ) ) ) {
			...
			$message = sprintf( esc_html__( '%1$s requires WPLoyalty to be installed and activated in order to be used.',
				'wp-loyalty-point-sharing' ), WLPS_PLUGIN_NAME );
			$allow_exit ? exit( esc_html( $message ) ) : self::adminNotice( esc_html( $message ), 'error' );
		...
		if ( ! self::isLoyaltyCompatible() ) {
			// translators: 1. %s will replace plugin name, 2. %s replace WooCommerce version
			$message = sprintf( esc_html__( '%1$s requires minimum WPLoyalty version %2$s', 'wp-loyalty-point-sharing' ), WLPS_PLUGIN_NAME, WLPS_MINIMUM_WLR_VERSION );
			$allow_exit ? exit( esc_html( $message ) ) : self::adminNotice( esc_html( $message ), 'error' );
	...
	protected static function isWooCompatible(): bool {
		...
		return (int) version_compare( $woo_version, WLPS_MINIMUM_WC_VERSION, '>=' ) > 0;
	}
	...
	protected static function isPHPCompatible(): bool {
		return (int) version_compare( PHP_VERSION, WLPS_MINIMUM_PHP_VERSION, '>=' ) > 0;
	}
	...
	protected static function isWordPressCompatible(): bool {
		return (int) version_compare( get_bloginfo( 'version' ), WLPS_MINIMUM_WP_VERSION, '>=' ) > 0;
	}
	...
	protected static function isLoyaltyCompatible(): bool {
		...
		return (int) version_compare( $wlr_version, WLPS_MINIMUM_WLR_VERSION, '>=' ) > 0;
	}
